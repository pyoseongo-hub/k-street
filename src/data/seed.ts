...
export interface Place {
  ...
  lng?: number;
  /**
   * 🌧️ **비를 안 맞고 둘러볼 수 있나** (2026-09-12).
   *
   * 값이 없으면 `isIndoor()`(src/lib/indoor.ts)가 **갈래로** 판단한다 —
   * 박물관·미술관은 정의상 실내다. 여기 적는 것은 **그 판단을 뒤집을 때**뿐이다:
   *   · `false` — 갈래는 박물관인데 실제로는 야외다(국립중앙박물관 전통염료식물원)
   *   · `true`  — 갈래는 시장·거리·꽃길인데 **지붕이 있다**
   *               (아케이드 시장 · 지하상가 · **서울식물원의 온실**)
   *
   * 🚨 **「식물원」이라는 말로 판단하지 말 것.** 위 두 줄에 식물원이 양쪽에 다 있다:
   *    전통염료식물원은 **통째로 야외**고, 서울식물원은 **온실이 건물**이다.
   *    이름이 아니라 **그 곳을 보고** 정한다.
   *
   * 🚨 **확인한 것만 적는다.** 「시장이니까 아케이드겠지」는 추측이다.
   *    비어 있으면 그 곳은 「비 오는 날」 테마에 **안 나온다.**
   *    빠지는 것이 틀리게 나오는 것보다 낫다 — 틀리면 손님이 젖는다.
   */
  indoor?: boolean;
  confirmed: boolean;
}
...
export const FESTIVALS: Place[] = [
  ...
  // 🙈 화면에서 뺐다 (사용자 지시 2026-09-02: "노원도 구청 그때그때 서치 해야해
  //    일단 가려").
  //    **고정된 공식 안내가 없다** — 회차마다 구청이 공고를 따로 내서 그때그때
  //    찾아야 한다. 그러면 우리가 적어 둔 달·장소가 언제 낡았는지 알 수가 없다.
  //    dancingnowon.isweb.co.kr은 그 해 행사용으로 열리는 자리라 다음 회차에는
  //    다른 주소일 수 있다.
  ...
  // 🙈 화면에서 뺐다 (사용자 지시 2026-09-02: "다다도 가려").
  //    노원·도봉과 같은 줄에서 나온 판단이라 이유도 같게 적는다 — 회차마다
...
export const FLOWERS: Place[] = [
  ...
  // 🌧️ **온실이 있어서 비 오는 날에도 볼 것이 있다** (2026-09-12).
  //    사장님이 네이버 장소 화면을 보내 주셨다. 거기 적힌 사실:
  //      주소 강서구 마곡동로 161 · 9호선 양천향교역 8번 출구 917m(걸어서)
  //      09:30 문 엶 · 17:00 입장 마감 · **매주 월요일 휴관**(공휴일 제외)
  //      온실(주제원) 어른 5,000원 · 만 65세 이상·만 5세 이하 무료
  //      공식 https://botanicpark.seoul.go.kr/
  //    🚨 **값과 시간은 화면에 안 적는다** — 바뀌는 값이라 확인한 날짜 없이
  //       적으면 틀린 정보가 된다. 여기 주석으로만 남긴다(다음 사람이 다시 안 찾게).
  //    🚨 **「식물원이니까 야외」가 아니다.** 열린숲·호수원은 야외지만
  //       온실은 건물이다. 그래서 `note` 에 **「온실만 실내」**를 같이 적는다 —
  //       이 말이 빠지면 손님이 비 맞으며 공원을 걷는다.
  { id: id(), gu: "강서구", category: "flower", name: "서울식물원", note: "봄꽃길 175선 공식 예시 · 온실만 실내", indoor: true, confirmed: true },
...
export const FESTIVALS_ADDED: Place[] = [
  ...
  { id: id(), gu: "서초구", category: "festival", name: "차 없는 잠수교 뚜벅뚜벅 축제", startMonth: 9, endMonth: 10, dateLabel: "9~10월 매주 일요일", addr: "잠수교 및 반포한강공원 일원", officialUrl: "https://www.festa-ddooddoo.com/", note: "매주 일요일 잠수교에서 차를 뺀다. 푸드트럭·공연·미디어아트, 해 지면 달빛무지개분수. 입장 무료(음식은 유료)", monthSource: "서울시 미래한강본부·서울 축제 일정 등록 (2026 9.6–10.25, 매주 일요일)", confirmed: true },
