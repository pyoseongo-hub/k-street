...
//   개수와 정확히 맞았고, 안 맞은 세종은 **시·군·구가 없는 게 맞다** — 읍·면이 시
//   바로 밑이라 units 에는 그 읍·면 10곳이 들어 있다(위 units 설명 참고).
...
export interface City {
  ...
  col: number;
  /**
   * 그 도시를 나누는 **한 단계 아래 칸**. Place.gu 에 들어가는 값이다.
   * · 대도시 — 구·군 (부산 16곳)
   * · 도     — 시·군 (경기 31곳)
   * · 세종   — **읍·면** (10곳). 세종은 시·군·구 단계가 아예 없어서
   *            읍·면이 시 바로 밑이다. 빈 배열이 아니라 읍·면을 넣어 둔다 —
   *            Place.gu 자리에 실제로 들어갈 값이 그것이기 때문이다.
   */
...
export function sidoOf(gu: string, city?: string): string {
  if (city) {
    ...
    //     전부 통과하고 있었다).
    //    (세종은 units 에 읍·면이 들어 있다 — 비어 있는 도시는 지금 없다.
    //     units 가 빈 도시가 생기면 그 도시는 이 확인을 통과시킨다.)
