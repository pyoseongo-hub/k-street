    ...
    <section className="panel district-explorer">
      ...
      <div ref={map.ref} className="de-picker">
        ...
        {/* 👆 **지도 바로 위** 한 줄 (2026-09-05 사장님: "동네지도 사용 은 어떤건지
            한번에 이해하기 힘들수도 있다는 생각이드네 / 더 간단히 만들던가 설명을
            해야할거 같은데").

            설명은 원래도 있었다 — 그런데 **지도 아래**에 있었다. 벌집을 다 지나친
            다음에야 나오니, 헷갈릴 사람은 이미 헷갈린 뒤였다. 자리가 틀렸던 것이지
            글이 없던 게 아니다. **안내는 막히기 전에 보여야 한다.**

            그래서 한 문장을 둘로 갈랐다 — 「누르면 나온다」는 여기(지도 위)로,
            색 설명(진한 색·옅은 색)은 그 색이 실제로 보이는 지도 아래에 남긴다.
            각자 설명하는 것 옆에 둔다.

            🚫 「더 간단히 만들까」도 생각했지만 벌집 자체는 안 건드린다 —
               지금 배치가 실제 서울과 대충 맞아서(도봉·노원이 위, 강남·송파가 아래)
               모양을 바꾸면 그 단서까지 잃는다. 모르는 건 **눌러도 되는지**였지
               칸이 복잡해서가 아니었다. */}
        <p className="hex-hint">👆 {t.tapDistrictHint}</p>
