    ...
    <div className="place-directions">
      ...
      <div className="map-directions-row">
        ...
        <button type="button" className="map-btn map-btn--luggage" onClick={() => setLuggage(true)}>
          {/* 🚻♿🛗 **여기만은 그림을 쓴다** (사장님 지시 2026-09-15:
                 *"텍스트 짐보관 아이콘으로. 화장실 휠체어나 엘리베이터 이걸로"*).

              ⚠️ 이 저장소는 2026-09-05에 **단추 그림을 전부 뺐다** — 사장님이
                 「아이콘 몬지 모르겠어」라고 하셨다(HomeSwitch 주석). 그 규칙을
                 뒤집는 게 아니라, **이 셋은 다른 종류**라서 넣는다:
                   · 🍂(계절)·🗺️(동네)는 **뜻을 정해야 읽히는** 그림이었다.
                   · 🚻·♿·🛗 는 **역 안내판에 실제로 붙어 있는 국제 픽토그램**이다.
                     손님은 공항에서·역에서 이미 이 그림을 보고 왔다.
              🌏 그리고 이 칸의 쓸모가 바로 그것이다 — 눌러서 가는 네이버 화면이
                 **한국어**라 손님이 글자를 못 읽어도 **그림표로 읽는다.**
                 단추의 그림과 그 화면의 그림이 같아야 이어진다.
              📝 글자는 남긴다. 그림만 두면 「어느 역?」이 안 보이고, 옆 단추 둘이
                 글자라 혼자만 그림이면 고장 난 것처럼 보인다.

              🛅 **짐 보관을 맨 앞에 둔다** (사장님 2026-09-15: *"짐보관도 넣어야해"*).
                 이름이 「짐 보관」에서 「역 편의시설」로 바뀌었으니, 원래 그걸 찾던
                 손님이 길을 잃으면 안 된다. 🛅 는 **Left Luggage 국제 표지**라
                 역·공항에 그대로 붙어 있는 그림이다 — 🧳(여행가방)가 아니다.
                 순서도 뜻이다: 🛅(짐) 다음에 ♿🛗🚻(교통약자·편의). */}
          <span className="map-btn-pictos" aria-hidden="true">
            🛅♿🛗🚻
          </span>
          {t.luggageLinkLabel}
        </button>
