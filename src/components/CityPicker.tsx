...
//
// ── 🚨 왜 지금은 「단추」가 아닌가 ────────────────────────────────────────
//   지금 열린 도시는 **서울 하나뿐**이다. 고를 것이 하나면 고르는 게 아니다.
//   눌러도 아무 일 없는 단추를 놓으면 손님은 앱이 고장 난 줄 안다 —
//   아래 탭에서 이미 한 번 겪은 일이다(App.tsx: *"단추는 없애든 되게 하든
//   둘 중 하나여야 한다"*).
//   → 열린 도시가 **둘 이상이 되는 순간** 칸이 저절로 단추가 된다(아래 pickable).
...
export default function CityPicker({ onPick }: { onPick?: (key: string) => void }) {
  ...
  // 🔢 열린 도시가 둘 이상일 때만 고를 수 있다(머리말 참고).
  const pick = CITIES.filter((c) => c.status === "공개").length > 1 ? onPick : undefined;
