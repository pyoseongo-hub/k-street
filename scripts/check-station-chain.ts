...
// 🕵️ **「역이 없다」는 답은 도심에서 의심해 볼 것** (2026-09-15).
//
//    삼청동길이 그랬다. 카카오에 「삼청동길」이 있긴 한데, 그게 **북악산 자락의
//    산길**(삼청동 산 2-1)이라 안국역에서 1.6km 였다. 이름은 글자까지 똑같아서
//    검사를 통과했지만, 단풍길이 가리키는 「경복궁 담장과 화랑·공방이 있는 그 길」이
//    아니다. 결과는 사장님이 처음 짚으신 것과 똑같은 「가까운 역 없음」이다.
//
//    기계가 이걸 이름만으로 가려낼 방법은 없다. 그래서 **의심스러운 것을 세워 둔다** —
//    산길은 진짜로 역이 멀지만, 도심 길이 여기 섞여 있으면 좌표가 틀린 것이다.
//    🚨 막지는 않는다. 진짜로 역이 없는 곳이 있기 때문이다(북한산길·안산산책길…).
const 역없음 = ALL_PLACES.filter((p) => ROWS[p.id]?.none).map((p) => `${p.category}/${p.gu} ${p.name}`);
if (역없음.length) {
  console.log(`\n🕵️ 1.5km 안에 역이 없다고 나온 곳 ${역없음.length}곳 — **눈으로 한 번 훑을 것.**`);
  console.log(`   산길이면 맞는 답이다. 도심 길이 섞여 있으면 좌표가 남의 곳에 붙은 것이다.`);
  for (const n of 역없음) console.log(`   · ${n}`);
}
