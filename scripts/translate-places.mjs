...
/**
 * 🀄 **구글이 「뜻」으로 옮겨 놓은 이름을 걷어낸다.**
 *
 * 무엇을 어떻게 가려내는지는 scripts/lib/cjk-name-rules.mjs 에 적어 뒀다 —
 * 그 파일을 **감사(check-cjk-names.mjs)와 같이 쓴다.** 잣대가 둘이면
 * 저장 쪽과 검사 쪽이 어긋나, 걸러 낸 줄 알았는데 안 걸러진 자료가 남는다.
 *
 * 🚨 지우기만 한다. 맞는 한자를 지어내지 않는다 — 앱이 영어로 대신 보여 준다.
 */
function dropBrokenCjkNames(store) {
  ...
  if (broken.length) {
    ...
    const what = Object.entries(by).map(([k, n]) => `${k} ${n}`).join(" · ");
    console.log(`🀄 이름이 아니라 낱말로 옮겨진 것 ${broken.length}개를 걷어냈다 (${what}) — 영어로 대신 보여 준다`);
  }
