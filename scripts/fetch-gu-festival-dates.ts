...
/**
 * 🎪 **관광객이 갈 만한 것인가** — 「우리에게 없는 축제」 목록에 넣을지 가른다.
 *
 * 🚨 **예전에는 `CODENAME.startsWith("축제")` 하나로 걸렀다. 그게 구멍이었다**
 *    (2026-09-15에 잡았다).
 *
 *    사장님이 인스타에서 **「차 없는 잠수교 뚜벅뚜벅 축제」**를 보고 물어봤는데,
 *    우리 앱에도 없고 **이 목록에도 없었다.** 하루 19,532줄을 받아 그중 서울에서
 *    아직 안 끝난 것이 406개인데, 「축제」 분류만 보느라 **43개만 보고 363개를
 *    안 보고 있었다.**
 *
 *    바로 위 `fetchAllEvents` 주석이 이미 경고하고 있던 것과 같은 이야기다 —
 *    *"같은 축제가 해마다 다른 분류로 등록된다"*. 대조할 때는 분류를 안 보면서
 *    **사람에게 보여 줄 때만 분류로 걸렀으니**, 분류가 어긋난 축제는 대조에서도
 *    안 걸리고(우리에게 없으니) 목록에서도 안 보였다. 이중으로 사라진 것이다.
 *
 * ✅ 그래서 두 갈래로 건진다:
 *    ① 분류가 「축제」로 시작하는 것 — 예전 그대로
 *    ② 분류가 무엇이든 **이름에 축제·페스티벌이 들어가고 야외에서 열리는 것**
 *
 *    ②에 야외 조건을 붙이는 이유 — 「~축제」라는 이름의 공연장 안 프로그램이
 *    수백 건이다. 그것까지 넣으면 목록이 길어져 **아무도 안 읽는다.**
 *    이 목록이 석 달간 안 읽혀 43곳이 쌓여 있던 것이 그 증거다.
 *    야외(공원·광장·한강·거리·섬·마을·길)여야 손님이 그냥 가서 볼 수 있다.
 */
const 야외 = /공원|광장|한강|거리|섬|마을|길|천|로$|대로|시장|놀이터|운동장|캠핑/;
const 축제이름 = /축제|페스티벌|페스타|festival|fest\b/i;
function 볼만한가(r: Row): boolean {
  if (r.CODENAME?.startsWith("축제")) return true;
  const title = (r.TITLE ?? "").normalize("NFC");
  const place = (r.PLACE ?? "").normalize("NFC");
  return 축제이름.test(title) && 야외.test(place);
}
...
// 🗺️ **25개 구를 돌면서 구별로 센다** (2026-09-15, 사장님: "축제 관련 루틴 만들어서
//    서울시 각 구 돌면서 정보 모아").
//
//    왜 구별로 세나 — **목록만으로는 빈 구가 안 보인다.** 43곳이 날짜순으로
//    늘어서 있으면 "많네" 하고 넘기게 되지, "강서구는 하나도 없네"는 안 보인다.
//    Kfood 의 coverage-map 에서 배운 것과 같다: **볼 자리를 만들어야 빈 곳이 보인다.**
const GU25 = [
  "종로구", "중구", "용산구", "성동구", "광진구", "동대문구", "중랑구", "성북구",
  "강북구", "도봉구", "노원구", "은평구", "서대문구", "마포구", "양천구", "강서구",
  "구로구", "금천구", "영등포구", "동작구", "관악구", "서초구", "강남구", "송파구", "강동구",
];
const ourByGu = new Map<string, number>();
for (const f of festivals) ourByGu.set(f.gu, (ourByGu.get(f.gu) ?? 0) + 1);
const newByGu = new Map<string, number>();
for (const r of notOurs) {
  const g = (r.GUNAME ?? "").normalize("NFC");
  newByGu.set(g, (newByGu.get(g) ?? 0) + 1);
}
const 빈구 = GU25.filter((g) => !(ourByGu.get(g) ?? 0));
console.log(`\n🗺️ 구별 축제 — 우리 것 / 포털에 있는데 우리에게 없는 것`);
for (const g of GU25) {
  const mine = ourByGu.get(g) ?? 0;
  const fresh = newByGu.get(g) ?? 0;
  // 우리 것이 0이면 🈳, 새로 들일 것이 있으면 🆕. 눈으로 훑을 수 있게 표시를 준다.
  const mark = mine === 0 ? "🈳" : fresh ? "🆕" : "  ";
  console.log(`   ${mark} ${g.padEnd(5)} 우리 ${String(mine).padStart(2)}곳 · 새로 ${String(fresh).padStart(2)}곳`);
}
if (빈구.length) {
  console.log(`\n🈳 **우리 앱에 축제가 한 곳도 없는 구 ${빈구.length}개** — ${빈구.join(" · ")}`);
  console.log(`   빈 구는 손님이 그 동네를 골랐을 때 아무것도 못 본다는 뜻이다. 여기부터 채운다.`);
} else {
  console.log(`\n✅ 25개 구 모두 축제가 하나 이상 있다 — 빈 구 없음.`);
}
// 🚧 **끝 표시는 어느 갈래로 가든 반드시 찍는다.**
//    워크플로가 `sed -n '/^🗺️ 구별 축제/,/^🗺️ (구별 표 끝)/p'` 로 이 토막만 떠 간다.
//    2026-09-15 에 여기서 데였다 — 끝 표시를 「빈 구는…」 줄로 삼았는데 빈 구가 0개라
//    그 줄이 안 찍혔고, sed 가 끝을 못 만나 **파일 끝까지 떠서 목록이 두 번 올라갔다.**
//    조건부로 찍히는 줄을 범위의 끝으로 쓰면 안 된다.
console.log(`🗺️ (구별 표 끝)`);

if (notOurs.length) {
  // ⚠️ 첫 낱말은 건드리지 말 것 — 워크플로가 이 줄에서 숫자를 뽑아 이슈 제목에 쓴다.
  console.log(`\n🆕 구청에는 있는데 우리 화면에 없는 축제 ${notOurs.length}곳 (야외에서 여는 것만):`);
