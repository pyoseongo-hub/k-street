...
/**
 * 🚨 **딱 맞아야 쓴다. 「포함」은 안 된다** (2026-09-12에 맛보기로 잡았다).
 *
 * 처음에는 `b.includes(a)` 로 느슨하게 맞췄다. 그랬더니 이런 것들이 통과했다:
 *   · 강남역지하도상가  → **「강남역 지하상가 개방화장실」**  ← 화장실이다
 *   · 남대문지하도상가  → **「남대문지하도상가상인연합」**    ← 상인회 사무실이다
 *   · 영등포역지하도상가 → **「영등포역지하도상가 솔브」**     ← 그 안의 매장 하나다
 * 셋 다 상가 자체가 아니다. 좌표를 그대로 썼으면 손님을 **화장실로** 보냈다.
 *
 * 이 저장소가 사진에서 똑같이 당했다 — 검색 첫 결과를 이름 대조 없이 써서
 * 남의 가게 사진이 붙었다. 「비슷하면 맞겠지」가 그 사고의 시작이다.
 *
 * 그래서 **정규화한 이름이 글자 하나까지 같을 때만** 쓴다.
 * 간판이 「지하도상가/지하쇼핑센터/지하상가」로 갈리는 것만 같은 말로 본다.
 */
