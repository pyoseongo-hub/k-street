...
// 문화시설(14)·여행코스(25)를 넓게 받은 뒤 제목에 키워드가 있는 것만 그
// 카테고리로 분류한다. 축제(15)만 그대로 전부 쓴다(전용 타입이라 필터가 필요 없음).
...
async function main() {
  ...
  console.log("서울 관광지(12) 받는 중...");
  const attractions = await fetchAllByContentType("12");
  console.log(`  → ${attractions.length}건`);

  console.log("서울 문화시설(14) 받는 중...");
  const cultural = await fetchAllByContentType("14");
  console.log(`  → ${cultural.length}건`);

  console.log("서울 여행코스(25) 받는 중...");
  const courses = await fetchAllByContentType("25");
  console.log(`  → ${courses.length}건`);

  const pool = [...attractions, ...cultural, ...courses];
